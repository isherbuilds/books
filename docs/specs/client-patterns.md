...
9. **Link Field**: a `Combobox` over the cached master, with rows from
   ...
   grant. Create stacks the master's own form and returns the saved row. A
   document's Party field lists the parties holding its role first and hides
   none; a party it creates starts with that role.
