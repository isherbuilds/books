...
import {
  ...
  readOrgSettings,
} from "@accly/api/lib/settings-cache";
...
test("parallel increments yield a gapless, unique sequence", async () => {
  ...
  const values = await Promise.all(
    Array.from({ length: 25 }, () =>
      db.transaction((tx) => nextCounter(tx, organization.id, "code")),
    ),
  );
...
test("a rolled-back transaction returns its number to the series", async () => {
  ...
  await db.transaction((tx) => nextCounter(tx, organization.id, "code"));

  await expect(
    db.transaction(async (tx) => {
      await nextCounter(tx, organization.id, "code");
      throw new Error("abort after increment");
    }),
  ).rejects.toThrow("abort after increment");

  const next = await db.transaction((tx) => nextCounter(tx, organization.id, "code"));
  expect(next).toBe(2);
});
