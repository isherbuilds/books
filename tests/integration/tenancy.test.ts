...
import { localMinute } from "@accly/api/lib/business-date";
...
import { addPendingItemCharge } from "../support/billing";
...
const GUARDED_CALLS = {
  ...
  "customer.search": (api, claim) => api.customer.search({ ...claim, query: "intrusion" }),
  "customer.get": (api, claim) => api.customer.get({ ...claim, customerId: Bun.randomUUIDv7() }),
  "customer.update": (api, claim) =>
    api.customer.update({
      ...claim,
  ...
  "customer.visits": (api, claim) =>
    api.customer.visits({ ...claim, customerId: Bun.randomUUIDv7() }),
  "customer.account": (api, claim) =>
    api.customer.account({ ...claim, customerId: Bun.randomUUIDv7() }),
  "item.list": (api, claim) => api.item.list({ ...claim }),
  "item.searchServices": (api, claim) =>
    api.item.searchServices({ ...claim, query: "intrusion", includeConsultation: false }),
...
test("customer rows are invisible from another org through search or get", async () => {
  const alice = await createTestUser("customer-scope-alice");
  const alpha = await createOrganization(alice, "customer-scope-alpha");
  ...
  const bobClient = clientFor(bob);
  expect((await bobClient.customer.search({ orgSlug: beta.slug, phone })).items).toHaveLength(0);
  await expectORPCCode(
    bobClient.customer.get({ orgSlug: beta.slug, customerId: customer.id }),
    "NOT_FOUND",
  );
  await expectORPCCode(
    bobClient.customer.visits({ orgSlug: beta.slug, customerId: customer.id }),
    "NOT_FOUND",
  );
  await expectORPCCode(
    bobClient.customer.account({ orgSlug: beta.slug, customerId: customer.id }),
    "NOT_FOUND",
  );
...
test("one client concurrently scopes customer calls to two organizations", async () => {
  const user = await createTestUser("customer-scope-multi");
  const one = await createOrganization(user, "customer-scope-one");
  const two = await createOrganization(user, "customer-scope-two");
  const api = clientFor(user);

  const [inOne, inTwo] = await Promise.all([
    api.customer.register({
      orgSlug: one.slug,
      name: "Customer In One",
      phone: "5551101",
      sex: "male",
...
test("item rows are invisible from another org and cannot be updated by foreign id", async () => {
  const alice = await createTestUser("item-scope-alice");
  ...
  const bobClient = clientFor(bob);
  expect((await bobClient.item.list({ orgSlug: beta.slug })).items).toEqual([]);
  await expectORPCCode(
    bobClient.item.update({
      orgSlug: beta.slug,
      itemId: item.id,
      name: item.name,
      code: item.code,
...
test("one client concurrently scopes item calls to two organizations", async () => {
  const user = await createTestUser("item-scope-multi");
...
test("one client concurrently scopes OPD calls to two organizations", async () => {
  ...
  const [customerOne, customerTwo] = await Promise.all([
    api.customer.register({
      orgSlug: one.slug,
      name: "OpdAppointment Customer One",
      phone: "5552402",
      sex: "other",
      dateOfBirth: "1996-08-27",
      dobEstimated: true,
    ...
    api.customer.register({
      orgSlug: two.slug,
      name: "OpdAppointment Customer Two",
      phone: "5552403",
      sex: "other",
      dateOfBirth: "1996-08-27",
      dobEstimated: true,
...
test("one client concurrently scopes report calls to two organizations", async () => {
  ...
  expect(gstOne.documents.map((document) => document.customerName)).toEqual([
    "Report One Customer",
  ]);
  expect(gstTwo.documents.map((document) => document.customerName)).toEqual([
    "Report Two Customer",
  ]);
