...
import {
  ...
  fiscalYearLabel,
  splitGst,
} from "@accly/api/lib/invoice-math";
...
test("rejects a discount greater than the subtotal", () => {
  expect(() => computeInvoiceLines([charge("a", "10.00")], parseMoney("10.01"))).toThrow();
});
...
test("derives the taxable and tax portions of a partial gross credit", () => {
  expect(derivePartialCredit(parseMoney("118.00"), "18.00")).toEqual({
    taxableValue: 10_000n,
    taxAmount: 1_800n,
    gross: 11_800n,
  });

  const inexact = derivePartialCredit(parseMoney("1.00"), "18.00");
...
test("parses and formats decimal money at the boundary", () => {
  expect(parseMoney("123")).toBe(12_300n);
  expect(parseMoney("123.4")).toBe(12_340n);
...
test("invoice balance accounts for credits, payments, and returned refunds", () => {
  const balance = calculateInvoiceBalance({
    grandTotal: parseMoney("500.00"),
    creditTotal: parseMoney("150.00"),
    paymentsTotal: parseMoney("400.00"),
    refundsTotal: parseMoney("25.00"),
  });
