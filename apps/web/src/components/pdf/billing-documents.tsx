...
export function InvoiceDocument({
      ...
      <DocumentShell invoice={invoice} title="Invoice" number={invoice.invoiceNumber} thermal>
        <Details
          rows={[
            ...
            { label: "Code", value: invoice.customerCode },
          ]}
        />
  ...
  const taxSummary = Array.from(
    lines.reduce((groups, line) => {
      const current = groups.get(line.taxRatePercent) ?? { taxable: 0n, tax: 0n };
      current.taxable += parseMoney(line.taxableValue);
      current.tax += parseMoney(line.taxAmount);
    ...
    <DocumentShell invoice={invoice} title="Invoice" number={invoice.invoiceNumber}>
      <section style={{ display: "flex", gap: 28, marginBottom: 20 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ color: colors.muted }}>Code {invoice.customerCode}</div>
          <div style={{ color: colors.muted }}>{invoice.customerPhone}</div>
              ...
              {taxSummary.map(([rate, amounts]) => {
                ...
                return (
                  <tr key={rate}>
                    ...
                    <td style={{ ...cellStyle, textAlign: "right" }}>
                      {formatDecimal(amounts.tax - cgst)}
                    </td>
                  </tr>
                );
              })}
    ...
    <DocumentShell invoice={invoice} title="Payment receipt" number={payment.receiptNumber}>
      ...
      <Details
        ...
        rows={[
          ...
          {
            label: "Received from",
            value: `${invoice.customerName} · Code ${invoice.customerCode}`,
          },
    ...
    <DocumentShell invoice={invoice} title="Refund voucher" number={refund.refundNumber}>
      ...
      <Details
        ...
        rows={[
          ...
          { label: "Refunded to", value: `${invoice.customerName} · Code ${invoice.customerCode}` },
          { label: "Against invoice", value: invoice.invoiceNumber },
