import { Sheet, SheetContent, SheetHeader, SheetTitle } from "@accly/ui/components/sheet";
import { ClientOnly, useBlocker } from "@tanstack/react-router";
import { useEffect, useRef, useState } from "react";

import { ConfirmDialog } from "@/components/confirm-dialog";
import { CustomerForm, type EditableCustomer } from "@/components/customer-form";

const DISCARD = {
  title: "Discard unsaved changes?",
  description: "The customer record has changes that have not been saved.",
      ...
      <ClientOnly fallback={null}>
        <Sheet open={open} onOpenChange={(next) => (next ? onOpenChange(true) : close())}>
          <SheetContent ref={panel}>
            <SheetHeader>
              <SheetTitle>{customer ? `Edit ${customer.code}` : "Register customer"}</SheetTitle>
            </SheetHeader>
            <CustomerForm
