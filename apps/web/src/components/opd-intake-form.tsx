...
import { OpdCustomerSearch, type SelectedCustomer } from "@/components/opd-customer-picker";
...
const intakeSchema = z
  .object({
    customer: z.custom<SelectedCustomer | null>(),
...
function serviceClaims(services: ServiceLine[]) {
  return services.map(({ itemId, qty }) => ({ itemId, qty }));
}
...
function quoteInput(orgSlug: string, values: IntakeValues) {
  if (values.when !== "now" || !values.customer || !values.practitionerId) {
    return null;
  }
...
// One behaviour whether or not the address named a customer: `?customerId` seeds the
// field and the field owns it from there.
function CustomerField({ orgSlug }: { orgSlug: string }) {
  const { control, setValue, setFocus } = useFormContext<IntakeValues>();
  const customer = useWatch({ control, name: "customer", exact: true });

  if (customer) {
    return (
      <div className="flex min-h-10 items-center gap-2 rounded-md bg-muted px-3">
        <Monogram label={customer.name} />
        <span className="flex min-w-0 flex-col leading-tight">
          <span className="truncate font-medium">{customer.name}</span>
          <span className="truncate font-mono text-muted-foreground">{customer.code}</span>
        </span>
        <Button
          type="button"
          size="sm"
          variant="ghost"
          className="ml-auto"
          onClick={() => setValue("customer", null, { shouldDirty: true })}
        >
          Change
        </Button>
      </div>
  ...
  return (
    <OpdCustomerSearch
      orgSlug={orgSlug}
      onSelect={(selected) => {
        setValue("customer", selected, { shouldDirty: true });
...
function ServicesFields({
  ...
  const lines = [
    ...
    ...services.map((service) => ({
      ...
      gross: when === "now" ? serviceGross.get(service.itemId) : undefined,
      editable: true,
    })),
  ];
  ...
  const setQty = (itemId: string, qty: number) =>
    form.setValue(
      "services",
      services.map((service) => (service.itemId === itemId ? { ...service, qty } : service)),
      { shouldDirty: true },
    );

  const remove = (itemId: string, editable: boolean) => {
    if (!editable) {
    ...
    form.setValue(
      "services",
      services.filter((service) => service.itemId !== itemId),
      { shouldDirty: true },
    );
  };
...
function IntakeSubmit({
  ...
  const hasCustomer = useWatch({
    control,
    name: "customer",
    exact: true,
    compute: (customer: SelectedCustomer | null) => customer !== null,
  });

  // Only a reason the operator cannot see on the form itself is worth words; an
  // unchosen customer is already obvious from the empty field above.
  const reason =
  ...
  const blocked = !hasCustomer || Boolean(reason) || (when === "now" && !quoteState.ready);
...
function FinancialAside({
  ...
  const customerName = useWatch({
    ...
    name: "customer",
    exact: true,
    compute: (customer: SelectedCustomer | null) => customer?.name ?? "—",
  });
      ...
      <Panel label={when === "now" ? "Payment" : "Booking"} minHeight="min-h-0" padded>
        ...
        {when === "later" ? (
          ...
          <dl className="grid gap-2">
            <SummaryRow term="Customer">{customerName}</SummaryRow>
...
export function OpdIntakeForm({
  ...
  const settleWalkIn = (settlement: SettlementDraft) => {
    ...
    if (!current.customer || !canSettleWalkIn || !quoteState.ready) return;
    createWalkIn.mutate({
      orgSlug,
  ...
  const submitValidated = intake.handleSubmit((current) => {
    ...
    if (parseMoney(quoteState.data.grandTotal) === 0n) {
      settleWalkIn({
        discountAmount: "0",
  ...
  // The overlay opens from a submit that already proved a customer is chosen, and the
  // form behind a modal cannot change it.
  const settlementCustomer = settlementOpen ? intake.getValues("customer") : null;
          ...
          <fieldset disabled={pending} className="contents">
            <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_18rem]">
              <div className="min-w-0 overflow-hidden rounded-lg border border-border bg-card">
                <IntakeSection title="Customer">
                  <CustomerField orgSlug={orgSlug} />
                </IntakeSection>
            ...
            {settlementOpen && settlementCustomer ? (
              <ClientOnly fallback={null}>
                <SettlementOverlay
                  quote={quoteState.data}
                  description={`${settlementCustomer.name} · walk-in now`}
