...
const visitsQuery = (orgSlug: string, customerId: string) =>
  orpc.customer.visits.infiniteOptions({
...
export function CustomerVisits({
  ...
  const visits = useInfiniteQuery(visitsQuery(orgSlug, customerId));
  ...
  return (
    ...
    >
      <ListState
        ...
        empty="This customer has no visits yet."
      >
        {rows.map((visit) => (
