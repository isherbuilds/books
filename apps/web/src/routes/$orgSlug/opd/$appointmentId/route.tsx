...
export const Route = createFileRoute("/$orgSlug/opd/$appointmentId")({
  ...
  loader: async ({ context: { queryClient }, params: { orgSlug, appointmentId } }) => {
    ...
    return {
      ...
      name: data.customer ? data.customer.name : data.appointment.callerName,
    };
  },
  head: ({ loaderData }) => ({
    meta: [
      {
        title: loaderData
          ? loaderData.tokenNumber != null
            ? `Token ${loaderData.tokenNumber} · ${loaderData.name ?? "Outpatient appointment"} · Accly Books`
            : `Booked · ${loaderData.name ?? "Outpatient appointment"} · Accly Books`
          : "Outpatient appointment · Accly Books",
      },
    ],
  }),
...
function OpdRecordLayout() {
  ...
  // customer and charge data visible after access was revoked.
  if (
    detail.error &&
        ...
        <PageHeader
          ...
          action={
            <>
              ...
              {isClinical ? (
                <Button
                  disabled={record.appointment.tokenNumber == null || !record.customer}
...
function OpdRecordFacts({ record }: { record: OpdRecordIdentity }) {
  ...
  return (
    <section className="grid gap-4 sm:grid-cols-2">
      {customer ? (
        <div className="flex flex-col gap-1">
          <h2 className="text-muted-foreground">Customer</h2>
        ...
        <div className="flex flex-col gap-1">
          ...
          <p className="text-muted-foreground">The customer record is linked at check-in.</p>
        </div>
      )}
      <div className="flex flex-col gap-1">
