...
export const Route = createFileRoute("/$orgSlug/billing/")({
  head: () => ({ meta: [{ title: "Billing · Accly Books" }] }),
...
function BillingIndexRoute() {
  ...
  return (
    <>
      <PageHeader title="Billing" description="Money owed to the business right now" />
      <PageBody>
        {summary ? (
          <Panel
            label="Today"
        ...
        <ListToolbar>
          <SearchInput
            ...
            placeholder="Search customer, code, or invoice number"
                  ...
                  <TableBody>
                    {refundData?.rows.map((row) => (
                      <TableRow key={row.invoiceId}>
                        ...
                        <TableCell className="max-w-0">
                          <p className="truncate font-medium" title={row.customerName}>
                            {row.customerName}
                          </p>
                          <p
                            className="truncate font-mono text-muted-foreground"
