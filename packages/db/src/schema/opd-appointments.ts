...
export const opdAppointments = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.customerId} is not null or (${table.callerName} is not null and ${table.callerPhone} is not null)`,
    ),
    check(
    ...
    check(
      ...
      sql`${table.status} <> 'checked_in' or (${table.customerId} is not null and ${table.tokenNumber} is not null and ${table.arrivedAt} is not null)`,
    ),
    check(
    ...
    // Carries the exact (business_date, id) keyset order the customer record pages on.
    index("opd_appointments_org_customer_date_idx").on(
      table.orgId,
      table.customerId,
      table.businessDate.desc(),
      table.id.desc(),
    ),
    index("opd_appointments_org_customer_arrived_idx")
      .on(table.orgId, table.customerId, table.practitionerId, table.arrivedAt)
