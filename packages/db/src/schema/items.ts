...
export const ITEM_CATEGORIES = ["consultation", "procedure", "lab", "radiology", "other"] as const;
...
export const items = pgTable(
  ...
  (table) => [
    ...
    check("items_unit_price_check", sql`${table.unitPrice} >= 0`),
    check(
      "items_tax_rate_check",
      sql`${table.taxRatePercent} >= 0 and ${table.taxRatePercent} <= 99.99`,
    ),
    unique("items_org_id_id_unique").on(table.orgId, table.id),
    uniqueIndex("items_org_code_idx").on(table.orgId, table.code),
    index("items_org_category_name_idx").on(table.orgId, table.category, table.name),
