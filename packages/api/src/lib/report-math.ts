...
export function buildTrialBalance({
  ...
  const rows = [...byAccount.values()]
    ...
    .map((row) => {
      ...
      return {
        ...
        name: row.name,
        type: row.type,
        openingDebit: formatMoney(line.openingDebit),
        openingCredit: formatMoney(line.openingCredit),
        debit: formatMoney(line.debit),
        credit: formatMoney(line.credit),
        closingDebit: formatMoney(line.closingDebit),
        closingCredit: formatMoney(line.closingCredit),
      };
    });
  ...
  return {
    ...
    rows,
    totals: {
      openingDebit: formatMoney(totals.openingDebit),
      openingCredit: formatMoney(totals.openingCredit),
      debit: formatMoney(totals.debit),
      credit: formatMoney(totals.credit),
      closingDebit: formatMoney(totals.closingDebit),
      closingCredit: formatMoney(totals.closingCredit),
    },
  };
}
...
export function buildBalanceSheet({
  ...
  const present = (rows: Line[]) =>
    rows.sort(byCode).map((row) => ({ ...row, balance: formatMoney(row.balance) }));
  ...
  return {
    ...
    totals: {
      assets: formatMoney(sum(assets)),
      liabilitiesAndEquity: formatMoney(sum(liabilities) + sum(equity)),
    },
  };
}
...
export function buildGstReport({
  ...
  const documents = [...documentMap.values()]
    ...
    .map((document) => ({
      ...
      taxableValue: formatMoney(document.taxableValuePaise),
      cgst: formatMoney(document.cgstPaise),
      sgst: formatMoney(document.sgstPaise),
      taxAmount: formatMoney(document.taxAmountPaise),
      gross: formatMoney(document.grossPaise),
    }));
  ...
  const rateSummary = [...rateMap.entries()]
    ...
    .map(([taxRatePercent, bucket]) => ({
      taxRatePercent,
      taxableValue: formatMoney(bucket.taxableValuePaise),
      cgst: formatMoney(bucket.cgstPaise),
      sgst: formatMoney(bucket.sgstPaise),
      taxAmount: formatMoney(bucket.taxAmountPaise),
    }));
  ...
  return {
    ...
    hsnSummary,
    totals: {
      taxableValue: formatMoney(totals.taxableValuePaise),
      cgst: formatMoney(totals.cgstPaise),
      sgst: formatMoney(totals.sgstPaise),
      taxAmount: formatMoney(totals.taxAmountPaise),
      gross: formatMoney(totals.grossPaise),
    },
  };
}
