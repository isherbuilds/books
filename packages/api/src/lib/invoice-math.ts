import { divideHalfUp, parseMoney } from "../core/money";
...
export function computeInvoiceLines(charges: ChargeInput[], discountPaise: bigint) {
  ...
  const lines = preparedLines.map(({ charge, lineSubtotal, allocatedDiscount }) => {
    ...
    const taxAmountPaise = divideHalfUp(
      taxableValuePaise * parseMoney(charge.taxRatePercent),
      10_000n,
    );
...
export function derivePartialCredit(gross: bigint, taxRatePercent: string) {
  const taxableValue = divideHalfUp(gross * 10_000n, 10_000n + parseMoney(taxRatePercent));
