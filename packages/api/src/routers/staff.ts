...
async function assertItemInScope(itemId: string, orgId: string): Promise<void> {
  const [row] = await db
    .select({ id: items.id })
    .from(items)
  ...
  const [, feeItems] = await Promise.all([
    ...
    feeItemIds.length
      ? db
          .select({ id: items.id })
          .from(items)
          .where(and(eq(items.orgId, orgId), inArray(items.id, feeItemIds)))
      : Promise.resolve([]),
...
export const staffRouter = {
  ...
  createDepartment: orgProcedure(
    ...
    if (input.defaultConsultFeeItemId != null) {
      await assertItemInScope(input.defaultConsultFeeItemId, scope.orgId);
    }
