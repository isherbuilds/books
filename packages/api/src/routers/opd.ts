...
import { OPD_APPOINTMENT_STATUSES, opdAppointments } from "@accly/db/schema/opd-appointments";
...
const bookInput = orgInput
  ...
  .superRefine((value, context) => {
    if (!value.customerId && (!value.callerName || !value.callerPhone)) {
      context.addIssue({
        code: "custom",
        path: ["callerName"],
        message: "Choose a customer or enter caller name and phone",
      });
    }
  });
...
export const opdRouter = {
  ...
  // The fee depends on whether this customer counts as a follow-up, which only the
  // server knows.
  quoteWalkIn: orgProcedure(
    { opd: ["read"], customer: ["read"] },
    orgInput.extend({
      customerId: z.string(),
      practitionerId: z.string(),
      services: serviceLines.default([]),
  ...
  createWalkIn: orgProcedure(
    { opd: ["create"], customer: ["read"], billing: ["write"] },
    orgInput.extend({
      customerId: z.string(),
      practitionerId: z.string(),
      settlement: z.object({
  ...
  checkIn: orgProcedure(
    { opd: ["update"] },
    appointmentIdInput.extend({ customerId: z.string().optional() }),
  ).handler(async ({ context, input }) => {
    const { scope } = context;
    const now = new Date();
    const [settings, [snapshot]] = await Promise.all([
      ...
      db
        .select({
          customerId: opdAppointments.customerId,
          ...
          search
            ? or(
                ...
                phoneSearch === undefined
                  ? undefined
                  : or(
                      ilike(sql`regexp_replace(${customers.phone}, '\\D', '', 'g')`, phoneSearch),
                      ilike(
      ...
      await Promise.all([
        appointment.customerId
          ? db
              ...
              .from(customers)
              .where(
                and(eq(customers.orgId, scope.orgId), eq(customers.id, appointment.customerId)),
              )
              .limit(1)
          : Promise.resolve([]),
        db
