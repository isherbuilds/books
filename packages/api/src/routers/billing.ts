...
import { nextCounter, type DbTransaction } from "@accly/db/counter";
...
async function issueInvoiceTx(
  ...
  const [appointmentAndCustomer] = await tx
    ...
    .innerJoin(
      customers,
      and(eq(customers.orgId, scope.orgId), eq(customers.id, opdAppointments.customerId)),
    )
  ...
  const pendingCharges = await tx
    ...
    .where(
      and(
        ...
        eq(charges.opdAppointmentId, appointmentAndCustomer.opdAppointmentId),
  ...
  const [invoice] = await tx
    ...
    .values({
      id: invoiceId,
      orgId: scope.orgId,
      opdAppointmentId: appointmentAndCustomer.opdAppointmentId,
      customerId: appointmentAndCustomer.customerId,
      invoiceNumber,
      fiscalYear,
      ...
      customerName: appointmentAndCustomer.customerName,
      customerCode: appointmentAndCustomer.customerCode,
      customerPhone: appointmentAndCustomer.customerPhone,
      customerAddress: appointmentAndCustomer.customerAddress,
  ...
  const [versionedAppointment] = await tx
    ...
    .where(
      and(
        ...
        eq(opdAppointments.id, appointmentAndCustomer.opdAppointmentId),
      ),
    )
  ...
  for (const payment of args.payments) {
    ...
    await postJournalEntry(tx, {
      ...
      lines: [
        ...
        { account: "customer_receivables", credit: inserted.amount },
      ],
    });
  ...
  recordRefund: orgProcedure(
    ...
    const refund = await db.transaction(async (tx) => {
      ...
      await postJournalEntry(tx, {
        ...
        timeZone: settings.timeZone,
        lines: [
          { account: "customer_receivables", debit: inserted.amount },
